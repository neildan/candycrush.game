/**
  |--------------------------------------------------------------------------
  | Código para NextU - Patrón módulo: Game
  |--------------------------------------------------------------------------
  |
  | Tiene las siguientes funcionalidades:
  |
  | 1) Los dulces se mueven con drag & drop.
  | 2) El título “Match Game” cambia de color, cada cierta cantidad aleatoria de segundos.
  | 3) El botón de "Iniciar", cambia su contenido a "Reiniciar" cuando se oprime.
  | 4) Tiene un temporizador a dos minutos, que inicia cuando se da en el botón "Iniciar".
  | 5) Cuando inicia el juego, se generan los dulces aleatoriamente, llenando todo el tablero.
  | 6) Cuando se presiona el botón de "Reiniciar", se elimina los dulces del tablero, el puntaje y los movimientos.
  | 7) Cuando el jugador realiza un movimiento, el contador de movimientos aumenta.
  | 8) Cuando se hace una combinación de 3 dulces o más en línea (Horizontal - Vertical), se generan nuevos dulces.
  | 9) (Pendiente) Se genera una animación cuando se hace una combinación.
  | 10) (Pendiente) Se muestra un efecto de gravedad, cuando se generan nuevos dulces a raíz de una combinación.
  | 11) Se aumenta el puntaje cuando sucede una combinación.
  | 12) El puntaje de una combinación es equivalente a la cantidad de dulces que participaron en dicha jugada.
  | 13) Cuando se acaba el tiempo, se elimina el tablero y el temporizado mediante una animación.
  | 14) Cuando se acaba el tiempo, se muestra el puntaje y cantidad de movimientos en toda la pantalla.
  | 15) Se utiliza la función "setTimeout de JavaScript" para retrasar la ejecución algunas funciones.
  |
  |--------------------------------------------------------------------------
  | Adicional
  |--------------------------------------------------------------------------
  |
  | 1) Muestra los 3 mejores puntajes obtenidos, único por cada dispositivo.
  | 2) Se modifica para que funcione responsive.
  | 3) Se agrega Service Worker y manifiesto para que funcione como PWA.
  | 4) (Pendiente) Se agrega instrucciones visuales.
  | 5) Se códifica en ingles porque se publicará públicamente.
  |
  |--------------------------------------------------------------------------
...
var Game = (function Game() {
     ...
     * @author Daniel Valencia <2020/07/07>
     */
    class TimerGame {
        /**
         * Get the timer
     ...
     * @author Daniel Valencia <2020/07/06>
     */
    function init() {
     ...
     * @author Daniel Valencia <2020/07/08>
     */
    function resetGame() {
    ...
    */
     ...
     * @author Daniel Valencia <2020/07/14>
     */
     ...
     * @return Boolean
     * @author Daniel Valencia <2020/07/09>
     */
    ...
    */
    ...
    function repeatedMovements(movement) {
        if (successfulMovements.length > 0) {
            ...
            if (movement.successRow) {
                successfulMovements.some(element => {
     ...
     */
    function addScore() {
        successfulMovements.forEach(move => {
